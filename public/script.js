...
class ChatClient {
    ...
    initSocketEvents() {
        ...
        this.socket.on('updateUserCount', (count) => {
            this.usersCountDiv.textContent = `Пользователи онлайн: ${count}`;
        });
    ...
    sendMessage() {
        ...
        if (text) {
            ...
            if (this.replyToId) {
                message.replyToId = this.replyToId; // Привязываем ID сообщения
                this.replyToId = null; // Сбрасываем после отправки
            }
    ...
    displayMessage(message) {
        ...
        if (message.replyToId) {
            const replyToMessage = document.querySelector(`[data-message-id="${message.replyToId}"]`);
            if (replyToMessage) {
                const replyPreview = replyToMessage.textContent.split(':').slice(1).join(':').trim();
                replyText = `<div class="reply-preview">Ответ на:[${replyPreview}</div>`;
            }
        }
        ...
        messageElement.innerHTML = `
            ...
            <button class="reply-button" onclick="client.replyToMessage('${message.id}')">Ответить</button>
        `;
